void rootlogon_Visby() {
  ...
  gROOT->ProcessLine(".L DataTrimVisby.C+"); //Load the classes that we want to use automatically - Convenient!
  gROOT->ProcessLine(".L ReadVisby.C+"); //The + means to recompile only if it changed sine last time
